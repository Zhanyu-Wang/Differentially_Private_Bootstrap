...
save_str <- "logistic_regression_comparison"
provide_PR_code_name <- "Ontario"
figure_width <- 10
figure_height <- 10
conf_level <- 0.9

linetype_values <- c('solid','dashed','dotted','dotdash','longdash','twodash','solid','dashed')
    ...
    for(ep in ep_list){
      for(B in B_list){
        ...
        if (B == B_list[1]){
          ...
          width_list = rbind(width_list, c(as.double(CIs[3,2:5]), n, ep, (CIs[3,4]-clean_width)/clean_width, 1, "DP-CI-ERM"))
