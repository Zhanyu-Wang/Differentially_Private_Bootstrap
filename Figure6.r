...
for(penalty_c in c(penalty_c)){
  for(n in n_list){
    for(ep in ep_list){
      ...
      if (ep == ep_list[1]){
        width_list = rbind(width_list, c(as.double(CIs[1,2:9]), n, "Bootstrap"))
      }
      width_list = rbind(width_list, c(as.double(CIs[3,2:9]), n, paste("DP-CI-ERM", sep="")))
      width_list = rbind(width_list, c(as.double(CIs[2,2:9]), n, paste("DP Bootstrap", sep="")))
    }
  }
}
width_list = as.data.frame(width_list)
colnames(width_list) = c("coverage_0.9", "rej_rate_0.9", "mean_width_0.9", "std_width_0.9",
                         "coverage_0.95", "rej_rate_0.95", "mean_width_0.95", "std_width_0.95",
                         "n","method")
width_list[, 1:(ncol(width_list)-1)] <- sapply(width_list[, 1:(ncol(width_list)-1)], as.double)
width_list$n = as.integer(width_list$n)
width_list$method = factor(as.character(width_list$method), level=c("Bootstrap", "DP Bootstrap", "DP-CI-ERM"))
width_list$err_width_0.9 = 2*width_list$std_width_0.9
width_list$err_width_0.95 = 2*width_list$std_width_0.95
width_list$err_coverage_0.9 = 2*sqrt((1-width_list$coverage_0.9)*width_list$coverage_0.9/nSIM)
width_list$err_coverage_0.95 = 2*sqrt((1-width_list$coverage_0.95)*width_list$coverage_0.95/nSIM)
width_list$err_rej_rate_0.9 = 2*sqrt((1-width_list$rej_rate_0.9)*width_list$rej_rate_0.9/nSIM)
width_list$err_rej_rate_0.95 = 2*sqrt((1-width_list$rej_rate_0.95)*width_list$rej_rate_0.95/nSIM)
...
p1 <- ggplot(data=width_list, aes(x = n, y = mean_width_0.9, group=method, lt=method, colour=(method))) +
  ...
  scale_x_log10(limits = c(900, 120000), breaks = n_list) +
  scale_color_manual(values=c('darkgoldenrod1','darkcyan','red')) +
  labs(title=" CI width ",
...
linetype_values2 <- c('solid','dashed')
  ...
  for(B in c(B)){
    for(n in n_list){
      for(ep in ep_list){
        ...
        CIs = read.csv(filename2)
        if (ep == ep_list[1]){
          width_list2 = rbind(width_list2, c(as.double(CIs[1,2:9]), n, penalty_c, B, "Bootstrap"))
        }
        width_list2 = rbind(width_list2, c(as.double(CIs[2,2:9]), n, penalty_c, B, paste("DP Bootstrap",sep="")))
      }
    }
  }
}
...
width_list2$method = factor(as.character(width_list2$ep), levels = c("Bootstrap", "DP Bootstrap", "DP-CI-ERM"))
width_list2$err_width_0.9 = 2*width_list2$std_width_0.9
width_list2$err_width_0.95 = 2*width_list2$std_width_0.95
width_list2$err_coverage_0.9 = 2*sqrt((1-width_list2$coverage_0.9)*width_list2$coverage_0.9/nSIM)
width_list2$err_coverage_0.95 = 2*sqrt((1-width_list2$coverage_0.95)*width_list2$coverage_0.95/nSIM)
width_list2$err_rej_rate_0.9 = 2*sqrt((1-width_list2$rej_rate_0.9)*width_list2$rej_rate_0.9/nSIM)
width_list2$err_rej_rate_0.95 = 2*sqrt((1-width_list2$rej_rate_0.95)*width_list2$rej_rate_0.95/nSIM)
...
p4 <- ggplot(data=width_list2, aes(x = n, y = mean_width_0.9, group=method, lt=method, colour=(method))) +
  ...
  geom_errorbar(aes(ymin=pmax(0.0004, mean_width_0.9-err_width_0.9), ymax=mean_width_0.9+err_width_0.9), width=.01,
                position=position_dodge(.01)) +
  scale_y_log10(limits = c(0.0004, 0.02), breaks = c(0.0005,0.002,0.01)) +
...
ggarrange(sp2, sp1, ncol = 2, nrow = 1, widths = c(1,2), labels = "auto")

figure_width <- 10
figure_height <- 2.8
ggsave("Figure6.pdf", width=figure_width, height=figure_height)
